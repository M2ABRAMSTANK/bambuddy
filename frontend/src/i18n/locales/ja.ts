export default {
  ...
  printers: {
    ...
    plateDetection: {
      ...
      decision: {
        title: '判定内容',
        backendOpencv: 'OpenCV基準画像比較',
        backendAi: 'AIビジョンモデル',
        verdict: '判定',
        reason: 'モデルの根拠',
        aiConfigHint: '以下の保存済み基準画像と検出領域はOpenCVバックエンドにのみ適用されます。AIバックエンドはカメラ画像全体を評価します。',
        modeLabel: 'このプリンターのバックエンド',
        modeGlobal: 'グローバル設定に従う',
        modeSaved: 'プリンターの検出バックエンドを更新しました',
      },
      confidence: '信頼度',
      difference: '差分',
  ...
  settings: {
    ...
    ambientDryingSustainedEnabled: '湿度の持続を要求',
    ambientDryingSustainedDescription: '環境乾燥を開始する前に、湿度が設定した時間にわたってしきい値を超え続けるまで待機します — AMSの蓋を開けたときの急上昇をやり過ごします。',
    ambientDryingSustainedMinutes: '持続時間（分）',
  ...
  bedcheckAi: {
    title: 'ビルドプレートチェック — AI バックエンド',
    description: '印刷開始前にビルドプレートが空かどうかを判定する方法を選択します:組み込みの OpenCV 比較か、AI ビジョンモデルか。',
    backendLabel: '検出バックエンド',
    backendOpencv: 'OpenCV(デフォルト)',
    backendAi: 'AI',
    ...
    apiKeyHint: 'ローカルサーバーで不要な場合は空のままにします。',
    apiKeyPlaceholder: 'エンドポイントがキーを要求しない場合は空のままにします',
    testButton: '接続テスト',
    testSuccess: 'バックエンドに到達しました — {{ms}}ms で判定を受信しました。',
    testFailed: 'AI バックエンドから判定を取得できませんでした。',
    privacyWarning: 'この URL はローカルネットワーク上にありません。ビルドプレートチェックが実行されるたびに、プリンターのカメラスナップショットがこのアドレスに送信されます。スナップショットを非公開に保つには、ローカルサーバー(例:自分のネットワーク上の Ollama)を使用してください。このエンドポイントを信頼する場合はそのまま続行できます。',
    printersTitle: '監視対象プリンター',
    printersHint: '印刷開始前にどのプリンターがビルドプレートチェックを実行し、それぞれどのバックエンドを使うかを設定します。',
    useGlobal: 'グローバル設定 ({{backend}})',
    noPrinters: 'プリンターが設定されていません。',
    printerUpdated: 'プリンターの検出バックエンドを更新しました',
    statusTitle: 'ステータス',
    globalBackend: 'グローバルバックエンド',
    notMonitored: '監視対象外',
  },
  makerworld: {
    title: 'MakerWorld',
